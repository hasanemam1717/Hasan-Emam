...
export default function TelegramWidget() {
  ...
  const handleOpenChat = () => {
    const url = `${TELEGRAM_LINK}?text=${encodeURIComponent(TELEGRAM_MESSAGE)}`;
    window.open(url, "_blank");
    setIsOpen(false);
  };

  return (
    <div className="fixed bottom-[117px] sm:bottom-[133px] right-3 sm:right-5 z-[60] flex flex-col items-end gap-3">
      {/* ── Chat Popup ── */}
      <AnimatePresence>
        {isOpen && (
          <motion.div
          ...
          >
            ...
            <div className="p-4">
              ...
              <p className="text-xs text-gray-400 dark:text-white/30 mb-4">
                Pick a quick option or send a custom message.
              </p>
              ...
              <div className="space-y-2">
                {quickReplies.map((reply) => (
                  ...
                  >
                    ...
                    <HiChevronRight
                      size={14}
                      className="text-gray-300 dark:text-white/20 group-hover:text-sky-500 transition-colors"
                    />
                  </button>
                ))}
              </div>
              ...
              <button
                ...
                className="w-full mt-3 px-3 py-2.5 text-xs font-semibold text-white bg-gradient-to-r from-sky-500 to-blue-600 rounded-xl hover:from-sky-600 hover:to-blue-700 transition-all flex items-center justify-center gap-2"
              >
                <HiChat size={14} />
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.92 }}
        className="relative w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-gradient-to-br from-sky-500 to-blue-600 shadow-lg shadow-sky-500/20 hover:shadow-sky-500/40 flex items-center justify-center transition-all duration-300 hover:scale-105"
        aria-label={isOpen ? "Close Telegram chat" : "Open Telegram chat"}
      >
        {isOpen ? (
          <HiX className="text-white" size={24} />
        ) : (
          <FaTelegramPlane className="text-white ml-0.5" size={22} />
        )}

        {/* ── Pulse ring ── */}
        <span className="absolute inset-0 rounded-full bg-sky-500/40 animate-ping opacity-40" />
      </motion.button>
    </div>
  );
}
